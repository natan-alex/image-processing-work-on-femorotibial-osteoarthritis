...
class ModelService:
    ...
    @staticmethod
    def create_rfc_model(folder_path: str):
        #carregamento e pre-processamento de dados para treinamento
        #print(folder_path)
        path=os.path.join(folder_path,"train")
        path2=path.replace('/','\\')
        cls_dir=os.listdir(path2)

        print(cls_dir)
        X=[]
        y=[]
        for cls in cls_dir:
                path3=os.path.join(path2,cls)
                path3_5=path3.replace('/','\\')
                cls_img= os.listdir(path3_5)
                #print(cls_img)
                for image in cls_img:
                        path4=os.path.join(path3,image)
                        path4_5=path4.replace('/','\\')
                        img=cv2.imread(path4_5, cv2.IMREAD_GRAYSCALE)
