...
class FoldersAndFilesService:
    ...
    def read_model_related_folders_and_files() -> ReadDirResult:
        ...
        try:
            ...
            if not FoldersAndFilesService._are_folder_subfolders_valid(folder_path):
                return ReadDirResult(error=FoldersAndFilesService.subfolders_error_message)

            return ReadDirResult(
                folder_classes_and_files=FoldersAndFilesService
        ...
        for subfolder in list(ExpectedSubfolders):
            ...
            for clasz in list(ModelClasses):
                ...
                folder_classes_and_files[subfolder.value][clasz.value] = list(filter(
                    FoldersAndFilesService._is_file_an_allowed_image, class_folder_content))
